...
import { db, groups, Store, users } from '../src/storage.js';

const { smsg, getSender, getGroupMetadata } = Func;

const PLUGIN_DIR = './helpers/Plugins';
...
/*!====[ Pengaturan & prefix ]====!*/
// src/set/config.json. Bisa diedit manual atau lewat command, dan langsung dipakai tanpa restart.
// `prefix` berupa regex, tanpa ^. Tambah kunci baru (coowners, public, dst.) di objek bawah ini.
const CONFIG = './src/set/config.json';
export const settings = new Store(CONFIG, { prefix: '[.!#/]', owners: [] as string[] });
settings.flush(); // tulis kunci default ke file supaya kelihatan dan mudah diedit

// Yang dipantau folder-nya, bukan file: Store menyimpan lewat rename, dan itu
// membuat pemantau yang menempel di file berhenti bekerja.
let cfgTimer: ReturnType<typeof setTimeout> | undefined;
try {
  watch(dirname(CONFIG), (_event, name) => {
    if (name !== basename(CONFIG)) return;
    clearTimeout(cfgTimer);
    cfgTimer = setTimeout(() => {
      try {
        Object.assign(settings.data, JSON.parse(readFileSync(CONFIG, 'utf-8')));
      } catch {
        console.error('[handler] config.json tidak valid, perubahan diabaikan');
      }
    }, 300);
  });
} catch {}
...
export async function runEvent(name: string, data: Record<string, any> = {}) {
  for (const p of events.get(name) ?? []) {
    try {
      await p.run({ ...data, Func, users, groups, db, settings });
    } catch (e) {
      console.error(`[plugin ${p.file}]`, e);
    }
  }
